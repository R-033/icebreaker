...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		[HideInInspector]
		public bool AllowTransformationSwitch;
		...
		void OnPostRender()
		{
			...
			if (YAllowed || transformType != TransformType.Move)
			{
		...
		{
			if (AllowTransformationSwitch)
			{
				transformType = Input.GetKey(KeyCode.LeftShift) ? TransformType.Rotate : TransformType.Move;
			}
		...
		void TransformSelected()
		{
			...
			{
				if(nearAxis != Axis.None && Input.GetMouseButtonDown(0) && (YAllowed || nearAxis != Axis.Y || planeAxis == Axis.Y || transformType != TransformType.Move))
				{
