...
namespace Hypercooled.Carbon.Core
{
	public class TrackUnpacker : Shared.Core.TrackUnpacker
	{
		...
		public TrackUnpacker(string folder, string globalB, string lxry) : base(Game.Carbon, folder, globalB, lxry)
		{
		...
		protected override void RegisterStreamParsers(BlockReader reader)
		{
			...
			//reader.RegisterParser((uint)BinBlockID.WorldAnimHeader, () => new Parsers.WorldAnimHeaderParser());
			//reader.RegisterParser((uint)BinBlockID.WorldAnimMatrices, () => new Parsers.WorldAnimMatrixParser());
			//reader.RegisterParser((uint)BinBlockID.WorldAnimRTNode, () => new Parsers.WorldAnimRTNodeParser());
			//reader.RegisterParser((uint)BinBlockID.WorldAnimNodeInfo, () => new Parsers.WorldAnimNodeInfoParser());
			//reader.RegisterParser((uint)BinBlockID.WorldAnimPointer, () => new Parsers.WorldAnimPointerParser());
		}
		...
		protected override void ResolveStreamParsers(List<IParser> iparsers, ushort sectionNumber)
		{
			...
			foreach (var iparser in iparsers)
			{
				switch ((BinBlockID)iparser.BlockID)
				{
					...
					case BinBlockID.WorldGridMaker: this.UnfoldWorldGridMaker(iparser); break;
		...
		private void UnfoldScenerySection(IParser iparser)
		{
			...
			{
				userInfo = this.CreateNewUserInfo(Game.Carbon, scenery.SectionNumber);
			}
			...
			for (int i = 0; i < scenery.LightTextureCollections.Count; ++i)
			{
				var lightTexture = scenery.LightTextureCollections[i];
				var sceneryObject = userInfo.SceneryObjects[i] as SceneryObject;

				sceneryObject.LightTextureKey1 = lightTexture.LightTexture1.Key;
				sceneryObject.LightTextureKey2 = lightTexture.LightTexture2.Key;
				sceneryObject.LightTextureKey3 = lightTexture.LightTexture3.Key;
				sceneryObject.DirectionalMapKey = lightTexture.DirectionalMap.Key;
			}
		}
		private void UnfoldLightSourcePack(IParser iparser)
		{
			...
			{
				userInfo = this.CreateNewUserInfo(Game.Carbon, parser.SectionNumber);
			}
		...
		private void UnfoldWorldGridMaker(IParser iparser)
		{
