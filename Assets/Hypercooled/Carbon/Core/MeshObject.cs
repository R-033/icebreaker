...
namespace Hypercooled.Carbon.Core
{
	public class MeshObject : Shared.Core.MeshObject // 0x80134010
	{
			...
			foreach (var submesh in this.m_submeshes)
			{
				switch (submesh.DXShader)
				{
					case DXShaderType.WorldBoneShader:
						...
						goto case DXShaderType.WorldReflectShader;
					...
					case DXShaderType.CARNORMALMAP:
						result.HasTangents = true;
						goto case DXShaderType.WorldShader;

					case DXShaderType.WorldShader:
					case DXShaderType.GLASS_REFLECT:
					case DXShaderType.WATER:
					default:
						result.HasUV1 = true;
						break;
				}
		...
		private unsafe void ReadVertexBufferInfo(StructInfo info)
		{
			...
			for (int i = 0, vertIndex = 0; i < this.m_submeshes.Length; ++i)
			{
				...
				fixed (float* ptr = buffer)
				{
					...
					{
						case DXShaderType.WorldBoneShader:
							currentOffset = this.ReadWorldBoneSubMesh(ptr, currentOffset, vertIndex, submesh.NumVertices, info);
							break;
		...
		private unsafe int ReadWorldBoneSubMesh(float* ptr, int offset, int start, int count, StructInfo info)
		{
			for (int i = start; i < start + count; ++i)
			{
				var vx = ptr[offset++];
