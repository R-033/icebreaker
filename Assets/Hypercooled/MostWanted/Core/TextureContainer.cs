...
namespace Hypercooled.MostWanted.Core
{
	public class TextureContainer : Shared.Core.TextureContainer
	{
		...
		private void ReadHeaderInfo(BinaryReader br)
		{
			...
			if (this.Header.Version != Shared.Textures.TexturePackVersion.MostWanted)
			{
				Debug.LogWarning($"Texture pack has an invalid version! Got {this.Header.Version}, needs {(int)Shared.Textures.TexturePackVersion.MostWanted}");
			}
		}
