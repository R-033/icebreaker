...
namespace ELFSharp.ELF
{
    public sealed class ELF<T> : IELF where T : struct
    {
        ...
        private Section<T> GetSectionFromSectionHeader(SectionHeader header)
        {
            ...
            if (header.Name == ".data")
            {
                Main.ELFData_Offset = (int) header.Offset;
                Main.ELFData_SizeOffset = (int) header.SizeOffset;
            } else if (header.Name == ".shstrtab")
            {
                Main.shstrtab_offset = (int) header.OffsetOffset;
            } else if (header.Name == ".strtab")
            {
                Main.strtab_offset = (int) header.OffsetOffset;
            } else if (header.Name == ".symtab")
            {
                Main.symtab_offset = (int) header.OffsetOffset;
            } else if (header.Name == ".rel.data")
            {
                Main.rel_data_offset = (int) header.OffsetOffset;
            }
            switch(header.Type)
            {
                ...
                case SectionType.SymbolTable:
                    SymbolTable<T> s = new SymbolTable<T>(header, reader, objectsStringTable, this);
                    symtable = s;
                    returned = s;
                    break;
        ...
        private SimpleEndianessAwareReader ObtainEndianessAwareReader(Stream stream)
        {
            ...
            switch (endianessByte)
            {
                ...
                case 2:
                    ...
                    break;
                default:
                    throw new ArgumentException($"Given ELF file uses unknown endianess {endianessByte}.");
            }
